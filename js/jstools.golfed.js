...
let p=e=>typeof e,w=(c,...a)=>new c(...a),four=(t,o=0,i=o)=>w(Array,t).fill(i++),OF=(a,b)=>a instanceof b;
...
export let warn=(str,...sels)=>(clearWarn(...sels),sels.map(s=>el=s,p(s)[1]=="t"?el=document.querySelector(s):0,el.append(createElement("warn",{innerHTML:str})))),
clearWarn=(...sels)=>{
    sels.map(s=>{
        el=s;
...
error=(str,...selectors)=>{
    ...
    selectors.map(s=>(el=s,p(s)[1]=="t"&&(el=document.querySelector(s)),el.append(w.cloneNode(true))));
},
clearError=(...selectors)=>selectors.map(s=>{let el=s;p(s)[1]=="t"?el=document.querySelector(s):0;[...el.children].map(e=>e.tagName.toLowerCase()=="error"?e.remove():0);}),
hide=(...selectors)=>selectors.map(s=>(p(s)[1]=="t"?document.querySelector(s):s).classList.add("hidden")),
show=(...selectors)=>selectors.map(s=>(p(s)[1]=="t"?document.querySelector(s):s).classList.remove("hidden")),
clear=(...selectors)=>selectors.map(s=>{s=p(s)=="string"?document.querySelector(s):s;let arr=flattenChildNodes(s);arr.reverse().map(e=>e.remove?.call(e)),s.innerHTML=""}),
disable=(message,...selectors)=>selectors.map(s=>(p(s)[1]=="t"?document.querySelector(s):s).setAttribute("disabled",message)),
enable=(...selectors)=>selectors.map(s=>(p(s)[1]=="t"?document.querySelector(s):s).removeAttribute("disabled")),
tabColor=color=>{
    let valid=c=>c.match(/^(unset|initial|inherit)$/)?false:createElement("div",{style:{color:c}}).style.color!==""
    if(!valid(color))return;
    let c=createElement("canvas",{width:1,height:1}),ctx=c.getContext("2d");ctx.fillStyle=color;ctx.fillRect(0,0,1,1);document.head.append(createElement(document.querySelector("link[rel=icon]")||"link",{href:c.toDataURL(),rel:"icon"}));
},
...
listAllColorsOnPage=_=>{
    ...
    let arr=[...w(Array,106).fill(0).map((e,n)=>"--color"+(n+1)),...w(Array,10).fill(1).map((e,n)=>"--transparent"+(n+1))],root=getComputedStyle(document.querySelector(":root")),els=flattenChildren(document.body).map(e=>[e,getComputedStyle(e)]);
    arr=arr.map(c=>{
...
Settings=class extends EventTarget{
    ...
    getSection(id){return this.sections.find(e=>e.config.id==id);}
    export(){
    ...
    static fromJson(jsontext){
        ...
        try{
            let json=JSON.parse(jsontext),validate=Joi.object({config:Joi.object({name:Joi.string().required()}).required(),sections:Joi.array().items(Joi.object({config:Joi.object({name:Joi.string().required(),id:Joi.string().required()}),options:Joi.array().items(Joi.object({config:Joi.object({name:Joi.string().required(),id:Joi.string().required(),type:Joi.string().required(),value:Joi.any(),values:Joi.array()}).required()})).required()})).required()}).validate(json);
            if(validate.error){
...
timeConversions=(_=>(s=t=>t*1000,m=t=>t*s(60),h=t=>t*m(60),d=t=>t*h(24),w=t=>t*d(7),y=t=>t*d(365),{seconds:s,minutes:m,hours:h,days:d,weeks:w,years:y}))();
